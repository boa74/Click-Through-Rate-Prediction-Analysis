...
-   **28 Independant variables** representing customer behavior and ad characteristics:
...
-   **Generating Dummy Variables:**
    ...
    -   I created dummy variables for 8 categorical variables. Initially, I attempted to use the recipe package, but it did not yield the desired results. As a result, I opted to use the fastDummies library, which worked effectively for this purpose.
...
-   **Column Name Formatting:**
    ...
    -   encountered errors when running the randomForest model due to issues with column names. To resolve this, I replaced spaces in column names with dots (e.g., Top Banner → Top.Banner). Additionally, hyphens (-) in column names caused issues, so I modified them as well (e.g., age45-75 → age45.75).
...
-   **Multicollinearity Check:**
    ...
    -   Pairwise correlations were computed, and variables with a correlation greater than 0.85 were removed to address redundancy. High correlations between variables can lead to multicollinearity, increasing the risk of overfitting in the prediction model.
...
-   **Data Splitting**
    ...
    -   The analysis dataset was split into training and testing sets to evaluate model performance. The model with the lowest RMSE on both the training and testing sets will be selected. Once the best model is identified, the analysis_data.csv file will be used as the training dataset, and the scoring_data.csv file will serve as the test dataset for final predictions.
...
- To identify the best-performing model based on RMSE comparison, I configured all tree-based models with cv=3 and ntree=100. Initially, the randomForest model failed to run due to computational overload. To address this, I adjusted the cross-validation parameter (cv) to 3 and limited the number of trees (ntree) to 100. For consistency and fair comparison, I rebuilt all models using these parameters.


-   **A. Hybrid Step wise Variable Selection:**

    -   Selected 13 variables using stepwise regression.
    -   Variables tested with consistent conditions across models.
    -   I evaluated the dataset using different methods, including Variance Inflation Factor (VIF) and Principal Component Analysis (PCA). However, both methods yielded similar results.
...
-   **D. Decision Trees and Tuned Trees:**
...
tuned_treel$results
tuned_treeree = rpart(CTR~.,data=analysis_result,cp = cvModel$bestTune$cp)
pred = predict(tuned_tree,newdata=scoring_result)
```

-   **E. Random Forest and Bagging**
...
rf_model <- randomForest(CTR ~ ., data = train, ntree = 100,importance=TRUE)
...
-   **F.Boosting and Gradient Boosting (GBM):**
...
# Define cross-validation settings and tuning grid
...
scoring_result <- read.csv('scoring_result.csv')
pred <- predict(cvModel, newdata = scoring_result)
```

-   **G. XG Boost:**

    -   The XGBoost model typically delivers superior performance due to its use of shrinkage and hyperparameter tuning for optimization. For this model, I utilized accumulated values from the previous model to enhance the tuning process.
...
best_nrounds <- tune_nrounds$best_iteration
...
dtest <- xgb.DMatrix(data = as.matrix(test_input))
